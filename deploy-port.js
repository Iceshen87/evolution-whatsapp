...
services:
  ...
  evolution-api:
    ...
    environment:
      ...
      - CACHE_REDIS_URI=${REDIS_URL:-redis://redis:6379}
        ...
        stream.on('close', () => {
            ...
            conn.exec('cd /opt/evolution-whatsapp && docker compose -f docker-compose.light.yml up -d', (err, stream) => {
                ...
                stream.on('close', () => {
                    ...
                    setTimeout(() => {
                        // 测试端口
                        conn.exec('curl -s http://localhost:8080 -H "apikey: 684de76250938ef254f136318374608b" | head -30', (err, stream) => {
                            stream.on('data', d => process.stdout.write(d.toString()));
                            stream.on('close', () => {
                                console.log('\n=== 完成 ===');
                                console.log('Evolution API: http://8.222.170.254:8080');
