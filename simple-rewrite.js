...
conn.on('ready', () => {
    console.log('=== 重新写入 docker-compose.yml ===\n');
    
    // 先删除旧配置
    conn.exec('rm -f /opt/evolution-whatsapp/docker-compose.light.yml', (err, stream) => {
        stream.on('close', () => {
            console.log('已删除旧配置');
  ...
  frontend:
    ...
    entrypoint: ["sh", "-c", "cp -r /usr/share/nginx/html/* /output/ && echo 'Done'"]
    mem_limit: 128m
